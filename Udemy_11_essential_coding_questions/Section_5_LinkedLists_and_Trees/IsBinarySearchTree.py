...
## compact solution from python book
def is_binary_tree_bst(node,low_range=float('-inf'),high_range=float('inf')):
	if not node:
		return True
	elif not low_range <= node.value <= high_range:
		return False
	else:
		return (is_binary_tree_bst(node.left, low_range, node.value) and 
						(is_binary_tree_bst(node.right, node.value, high_range)))
						
print(is_binary_tree_bst(head1))